<template>
    <Modal v-model="isPush" title="推送" width="1200" @on-cancel="doCancel">
        <!--按钮部分-->
        <Card>
            <!--表格部分-->
            <Row style="margin-bottom: 15px">
                <Col span="3">
                    <p style="font-weight: bolder;margin-top: 6px">选择要推送的用户</p>
                </Col>
...
<script>
    import {findUserList, pushArticleToSingle} from "../../libs/API/vipUser";
    export default {
        name: 'article-push',
        data () {
            return {
        ...
        methods: {
            ...
            async doArticlePush(params) {
                let data = {};
                data.userId = params.id;
                data.articleId = this.articleId;
                let res = await pushArticleToSingle(data);
                if(!res.code){
